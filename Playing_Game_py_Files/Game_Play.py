...
import random
import time
from Playing_Game_py_Files.Classes import Dot, Level, Sound

def play_game(screen, width, height, BLACK, WHITE, RED, BLUE):
 ...
 # Outer loop for game levels
    for i in range(1, 4):
     ...
     # Difficulty definition
        ...
        while enemy_health > 0 and my_health > 0:
            ...
            if time.time() > next_dot_time_red and time.time() > freeze_time:
                dot = Dot(random.randint(20, width - 20),
                          random.randint(100, height - 20), color=RED, alpha=0, radius=level.radius)
                dots.append(dot)
         ...
         # Mouse event handling
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEMOTION:
                    for dot in dots.copy():
                        if dot.color == BLUE and mouse_y <= 100:  # Defend opponent's attack
        ...
        if my_health <= 0:
            ...
            lose = lose_font.render("You Died!", True, RED)
    ...
    finish_time = round(time.time(), 2)
    time_score = int(finish_time - start_time)
    value = []
    value.append([my_health, time_score])
    return value